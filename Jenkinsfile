    ...
    stages {
        stage('Checkout') {
         ...
         stage('SonarQube Analysis') {
            steps {
                sh '/opt/sonar-scanner/bin/sonar-scanner  -Dsonar.projectKey=project -Dsonar.sources=. -Dsonar.host.url=http://localhost:9000 -Dsonar.login=sqp_3bc9819d208d136d55a9433c2f8a1d4dd26c0019'
            }
        }
        stage('login') {
            steps {
